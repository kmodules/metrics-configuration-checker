...
BIN      := metrics-configuration-checker
COMPRESS ?= no
...
lint: $(BUILD_DIRS)
	...
	@docker run                                                 \
	    ...
	    $(BUILD_IMAGE)                                          \
	    golangci-lint run --enable $(ADDTL_LINTERS) --enable gofmt,goimports,unparam --timeout=10m --skip-files="generated.*\.go$\" --skip-dirs-use-default
